...
begin
  ...
  // a função QuotedStr serve para colocar uma string entre aspas.

  // Se selecionou todos, não aplica o filtro Where
  if cbMunicipios.ItemIndex = 0 then
    ...
    // Senão, filtra somente os clientes do município selecionado
  Else
    FormRelat01.qryDados.SQL.Text := 'SELECT * FROM CLIENTE ' +
      'JOIN MUNICIPIO ON ( ' + 'CLIENTE.ID_MUNICIPIO = MUNICIPIO.ID_MUNICIPIO) '
  ...
  IF FormRelat01.qryDados.RecordCount = 0 Then
    ShowMessage('Não existem clientes no município selecionado')
  Else
